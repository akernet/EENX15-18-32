...
# Title: Fftfas
# Generated: Wed Apr 11 14:04:51 2018
##################################################
...
class fftfas(gr.top_block, Qt.QWidget):

    def __init__(self):
        gr.top_block.__init__(self, "Phase difference")
        Qt.QWidget.__init__(self)
        self.setWindowTitle("Phase difference")
        ...
        self.settings = Qt.QSettings("GNU Radio", "fftfas")
...
def main(top_block_cls=fftfas, options=None):
    ...
    #---------------------------------------------------------------
    def change_center_frequency_and_sleep():
        
        # Wait for user input
        raw_input('Press enter when you want to continue: ')
        print('Starting')
        ...
        sleep_time = 10
        first_center_freq = 1*10**9
        second_center_freq = 1.01*10**9
        
        while (1):
            
            tb.set_center_freq(first_center_freq)
            print('\n Current center frequency: {}'.format(tb.get_center_freq()))
            time.sleep(sleep_time)
        
            tb.set_center_freq(second_center_freq)
            print('\n Current center frequency: {}'.format(tb.get_center_freq()))
            time.sleep(sleep_time)
    
    thread.start_new_thread(change_center_frequency_and_sleep, ())
